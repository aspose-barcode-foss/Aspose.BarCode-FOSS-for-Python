...
class Ean13TextLayoutPolicy(TextLayoutPolicy):
    """Provide text layout behavior for EAN-13."""
    ...
    def create_layout(
        ...
        *,
        ...
        left_offset_x = (3 + 21) * mw
        right_offset_x = (50 + 21) * mw

        return TextLayout(
            segments=(
                TextSegment(text=display_text[0], anchor="middle", offset_x=d1_offset_x, offset_y=text_y),
                TextSegment(text=display_text[1:7], anchor="middle", offset_x=left_offset_x, offset_y=text_y),
                TextSegment(text=display_text[7:13], anchor="middle", offset_x=right_offset_x, offset_y=text_y),
            )
        )
